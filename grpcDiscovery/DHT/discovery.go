...
func NewServiceDiscovery(config *ServerConfig) (*ServiceDiscovery, error) {
	ds, err := NewNetServer(config.NodePort, config.ServerIp, config.ServerType, config.ServerName, config.GrpcSeverAddress)
	if err != nil {
		return nil, err
	}
	go func() {
...
func (s *ServiceDiscovery) GetGroup(groupName string) (RangeResponse, error) {
