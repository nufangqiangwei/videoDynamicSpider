...
func ParseSSURL(u string) (data *Shadowsocks, err error) {
	...
	parse := func(content string) (v *Shadowsocks, ok bool) {
		...
		username, _ = utils.Base64URLDecode(username)
	...
	if v, ok = parse(content); !ok {
		...
		l, err = utils.Base64StdDecode(l)
		if err != nil {
			l, err = utils.Base64URLDecode(l)
			if err != nil {
				return
			}
		}
...
func (s *Shadowsocks) ConfigurationMC(info PriorInfo) (c Configuration, err error) {
	...
	udpSupport := false
	var proxySettings *utils.ProxySettings
	var extraOutbounds []utils.OutboundObject
	var chain []string
	...
	case "v2ray-plugin":
		...
		proxySettings = &utils.ProxySettings{
			Tag: dialerTag,
		}
		streamSettings := &utils.StreamSettings{}
		...
		if s.Plugin.Opts.Tls == "tls" {
			...
			streamSettings.TLSSettings = &utils.TLSSettings{}
			// SNI
		...
		default:
			...
			streamSettings.WsSettings = &utils.WsSettings{
				Path: path,
		...
		extraOutbounds = append(extraOutbounds, utils.OutboundObject{
			Tag:      dialerTag,
			Protocol: "freedom",
	...
	return Configuration{
		CoreOutbound: utils.OutboundObject{
			...
			Protocol: "shadowsocks",
			Settings: utils.Settings{
				Servers: []utils.Server{v2rayServer},
			},
...
func (s *Shadowsocks) ConfigurationMT(info PriorInfo) (c Configuration, err error) {
	...
	udpSupport := true
	var v2StreamSettings *utils.StreamSettings
	var v2Mux *utils.Mux
	switch s.Plugin.Name {
	case "simple-obfs":
		host := s.Plugin.Opts.Host
		...
		v2StreamSettings = &utils.StreamSettings{}
		tcpSettings := utils.TCPSettings{
		...
		case "tls":
			...
			v2StreamSettings.TLSSettings = &utils.TLSSettings{}
			// SNI
	...
	case "v2ray-plugin":
		...
		default:
			...
			v2StreamSettings.WsSettings = &utils.WsSettings{
				Path: path,
				Headers: utils.Headers{
					Host: host,
				},
