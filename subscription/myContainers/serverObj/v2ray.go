...
func ParseVmessURL(vmess string) (data *V2Ray, err error) {
	...
	raw, err := utils.Base64StdDecode(vmess[8:])
	if err != nil {
		raw, err = utils.Base64URLDecode(vmess[8:])
	}
	if err != nil {
		...
		s, err = utils.Base64StdDecode(s)
		if err != nil {
			s, _ = utils.Base64URLDecode(s)
		}
...
func (v *V2Ray) Configuration(info PriorInfo) (c Configuration, err error) {
	...
	case "vmess", "vless":
		...
		if l := len([]byte(id)); l < 32 || l > 36 {
			id = utils.StringToUUID5(id)
		}
		core.StreamSettings = &utils.StreamSettings{
			Network: network,
		}
		switch strings.ToLower(v.Protocol) {
		case "vmess":
			if ok, t, err := utils.IsDatetimeSynced(); err == nil && !ok {
				return Configuration{}, fmt.Errorf("please sync datetime first. Your datetime is %v, and the "+
					"correct datetime is %v", time.Now().Local().Format(utils.DisplayFormat), t.Local().Format(utils.DisplayFormat))
			}
			security := v.Security
			if security == "" {
				security = "auto"
			...
			core.Settings.Vnext = []utils.Vnext{
				{
					Address: v.Add,
					Port:    port,
					Users: []utils.User{
		...
		case "grpc":
			...
			core.StreamSettings.GrpcSettings = &utils.GrpcSettings{ServiceName: v.Path}
		case "ws":
			core.StreamSettings.WsSettings = &utils.WsSettings{
				Path: v.Path,
		...
		case "mkcp", "kcp":
			core.StreamSettings.KcpSettings = &utils.KcpSettings{
				Mtu:              1350,
				Tti:              50,
				UplinkCapacity:   12,
		...
		case "tcp":
			if strings.ToLower(v.Type) == "http" {
				tcpSetting := utils.TCPSettings{
					ConnectionReuse: true,
					Header: utils.TCPHeader{
						Type: "http",
						Request: utils.HTTPRequest{
							Version: "1.1",
							Method:  "GET",
							Path:    strings.Split(v.Path, ","),
							Headers: utils.HTTPReqHeader{
		...
		case "h2", "http":
			if v.Host != "" {
				core.StreamSettings.HTTPSettings = &utils.HttpSettings{
					Path: v.Path,
					Host: strings.Split(v.Host, ","),
				}
			} else {
		...
		if strings.ToLower(v.TLS) == "tls" {
			...
			core.StreamSettings.TLSSettings = &utils.TLSSettings{}
			if v.AllowInsecure {
			...
			core.StreamSettings.XTLSSettings = &utils.TLSSettings{}
			// SNI
			if v.SNI != "" {
			...
			core.StreamSettings.RealitySettings = &utils.RealitySettings{
				ServerName:  v.SNI,
		...
		// Flow
		if v.Flow != "" {
			vnext := core.Settings.Vnext.([]utils.Vnext)
...
func (v *V2Ray) ExportToURL() string {
	...
	log.Warning.Printf("unexpected protocol: %v", v.Protocol)
	return ""
}
