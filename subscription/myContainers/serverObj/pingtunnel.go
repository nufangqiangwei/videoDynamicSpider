...
func ParsePingTunnelURL1(u string) (data *PingTunnel, err error) {
	...
	u, err = utils.Base64StdDecode(u)
	if err != nil {
		log.Warning.Printf("ParsePingTunnelURL1: %v", u)
	...
	passwd, err := utils.Base64URLDecode(subMatch[2])
	if err != nil {
		log.Warning.Printf("ParsePingTunnelURL1: %v", subMatch[2])
		err = fmt.Errorf("ParsePingTunnelURL1: %w", err)
		return
	}
	data = &PingTunnel{
