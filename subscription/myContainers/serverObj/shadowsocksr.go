...
func ParseSSRURL(u string) (data *ShadowsocksR, err error) {
	...
	parse := func(content string) (v ShadowsocksR, ok bool) {
		...
		pswd, _ := utils.Base64URLDecode(pre[5])
		add, _ := utils.Base64URLDecode(pre[0])
		remarks, _ := utils.Base64URLDecode(q.Get("remarks"))
		protoparam, _ := utils.Base64URLDecode(q.Get("protoparam"))
		obfsparam, _ := utils.Base64URLDecode(q.Get("obfsparam"))
	...
	if info, ok = parse(content); !ok {
		...
		content, err = utils.Base64StdDecode(content)
		if err != nil {
			content, err = utils.Base64URLDecode(content)
			if err != nil {
				return
			}
		}
