...
func deployWebSIteHtmlFile(gtx *gin.Context) {
	defer func() {
		if r := recover(); r != nil {
			utils.ErrorLog.Println(r)
			gtx.String(500, "Internal Server Error")
		}
	}()
	utils.Info.Println("开始读取前端文件")
	...
	for !readEOF {
		...
		case tar.TypeDir:
			...
			if err != nil {
				utils.ErrorLog.Println("上传前端文件。创建文件夹出错")
				utils.ErrorLog.Println(err)
				continue
			}
		case tar.TypeReg:
			...
			if err != nil {
				utils.ErrorLog.Println("上传前端文件。创建文件出错")
				utils.ErrorLog.Println(err)
				continue
			}
			// 将文件内容拷贝到目标文件
			if _, err := io.Copy(staticFile, tarRead); err != nil {
				utils.ErrorLog.Println("上传前端文件。写入文件出错")
				utils.ErrorLog.Println(err)
			}
			staticFile.Close()
		}
